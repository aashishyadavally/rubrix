...
def embedd_captions(captions_path,embeddings_folder):
    """
    ...
    ------
        ...
        embeddings_folder (string):
            path to the folder to store the numpy embeddings in
     
    Returns:
    --------
        ids_to_numpy_paths (dict):
            maps the image_ids to a lits of the absolute paths of the numpy embeddings of its captions
    """
    ...
    for caption_pair in ids_captions:
        ...
        if image_id in ids_to_numpy_paths.keys():
            ...
            numpy_path = embeddings_folder + "/" + image_id[:-4] + "_" + str( len(ids_to_numpy_paths[image_id]) + 1) + ".npy"
            ids_to_numpy_paths[image_id].append(numpy_path)
        else:
            #If this is the first time this image has been embedded, then _1
            numpy_path = embeddings_folder + "/" + image_id[:-4] + "_1.npy"
...
jsonembeddinglocations = str(pathfinder.get("storyteller","assets","data")) + "/imageEmbeddingLocations.json"

print(final_dict)

jsonfile = open(jsonembeddinglocations,'w')
